...
async function startScanner() {
  ...
  try {
    ...
    await scanner.start({ facingMode: { ideal: 'environment' } }, { fps: 10, qrbox: { width: 280, height: 160 } }, async decodedText => {
    ...
    els.scannerMessage.textContent = `เปิดกล้องไม่ได้: ${error.message || 'กรุณาตรวจสิทธิ์กล้อง'}`;
    await stopScanner();
  }
}
